...
def run_tests():
    ...
    demo_auth = post(f"{base_url}/api/auth/demo")
    ...
    otp_req = post(f"{base_url}/api/auth/send-otp", {"target": user["email"], "purpose": "login"})
    demo_otp = otp_req.get("demo_otp") or "123456"
    verify_res = post(f"{base_url}/api/auth/verify-otp", {"target": user["email"], "otp_code": demo_otp, "purpose": "login"})
    ...
    sample = post(f"{base_url}/api/datasets/sample", {"sample_key": "sales_data"}, token=token)
    ds_id = sample["id"]
    ...
    overview = get(f"{base_url}/api/visualization/overview/{ds_id}", token=token)
    ...
    profile = get(f"{base_url}/api/profiling/profile/{ds_id}", token=token)
    ...
    cleaned = post(f"{base_url}/api/cleaning/clean/{ds_id}", {
    ...
    corr = get(f"{base_url}/api/correlation/matrix/{ds_id}?method=pearson", token=token)
    print(f"[PASS] 8. Correlation Matrix: {len(corr['columns'])} numeric feature pairs evaluated")

    # 9. AI Strategic Insights & SWOT
    insights = post(f"{base_url}/api/ai/insights/{ds_id}", token=token)
    ...
    chat_res = post(f"{base_url}/api/chat/message", {
        "dataset_id": ds_id,
    ...
    train_res = post(f"{base_url}/api/ml/train/{ds_id}", {
        "target_col": "Sales",
    ...
    pred_res = post(f"{base_url}/api/ml/predict", {
        "dataset_id": ds_id,
    ...
    fc_res = post(f"{base_url}/api/forecasting/forecast/{ds_id}", {
        "date_col": "Date",
        "target_col": "Sales",
        "forecast_periods": 30
    }, token=token)
    ...
    anom_res = post(f"{base_url}/api/anomaly/detect/{ds_id}", {
    ...
    admin_ov = get(f"{base_url}/api/admin/overview", token=token)
